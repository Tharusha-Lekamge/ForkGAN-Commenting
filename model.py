...
def domain_agnostic_classifier(percep, options, reuse=False, name="percep"):
    with tf.compat.v1.variable_scope(name):
        if reuse:
            ...
            assert tf.compat.v1.get_variable_scope().reuse is False
        h1 = lrelu(
            instance_norm(conv2d(percep, options.df_dim * 4, name="d_h1_conv"), "d_bn1")
        )
        h2 = lrelu(
            instance_norm(conv2d(h1, options.df_dim * 2, name="d_h2_conv"), "d_bn2")
        )
        h3 = lrelu(
            instance_norm(conv2d(h2, options.df_dim * 2, name="d_h3_conv"), "d_bn3")
        )
...
def sce_criterion_list(logits, labels):
    ...
    for i in range(len(labels)):
        loss += tf.reduce_mean(
            tf.nn.sigmoid_cross_entropy_with_logits(logits=logits[i], labels=labels[i])
        )
...
class cyclegan(object):
    ...
    def discriminator(self, image, options, reuse=False, name="discriminator"):
        ...
        with tf.compat.v1.variable_scope(name):
            if reuse:
                ...
                assert tf.compat.v1.get_variable_scope().reuse is False
            images = dis_down(
                images, 4, 2, self.n_scale, options.df_dim, "d_h0_conv_scale_"
            )
            images = dis_down(
                images, 4, 2, self.n_scale, options.df_dim * 2, "d_h1_conv_scale_"
            )
            images = dis_down(
                images, 4, 2, self.n_scale, options.df_dim * 4, "d_h2_conv_scale_"
            )
            images = dis_down(
                images, 4, 2, self.n_scale, options.df_dim * 8, "d_h3_conv_scale_"
            )
    ...
    def _build_model(self):
        ...
        self.fake_B, self.rec_realA, self.realA_percep = self.generator(
            self.real_A, self.options, False, name="generatorA2B"
        )
        self.fake_A_, self.rec_fakeB, self.fakeB_percep = self.generator(
            self.fake_B, self.options, False, name="generatorB2A"
        )
        self.fake_A, self.rec_realB, self.realB_percep = self.generator(
            self.real_B, self.options, True, name="generatorB2A"
        )
        self.fake_B_, self.rec_fakeA, self.fakeA_percep = self.generator(
            self.fake_A, self.options, True, name="generatorA2B"
        )

        self.realA_percep_logit = self.domain_agnostic_classifier(
            self.realA_percep, self.options, False, name="percep"
        )
        self.realB_percep_logit = self.domain_agnostic_classifier(
            self.realB_percep, self.options, True, name="percep"
        )
        self.fakeA_percep_logit = self.domain_agnostic_classifier(
            self.fakeA_percep, self.options, True, name="percep"
        )
        self.fakeB_percep_logit = self.domain_agnostic_classifier(
            self.fakeB_percep, self.options, True, name="percep"
        )

        self.g_cls_loss = (
            sce_criterion(self.fakeA_percep_logit, self.A_label) * 0.5
            + sce_criterion(self.fakeB_percep_logit, self.B_label) * 0.5
        )
        self.cls_loss = (
            sce_criterion(self.realA_percep_logit, self.A_label) * 0.25
            + sce_criterion(self.realB_percep_logit, self.B_label) * 0.25
            + sce_criterion(self.fakeB_percep_logit, self.A_label) * 0.25
            + sce_criterion(self.fakeA_percep_logit, self.B_label) * 0.25
        )
        ...
        self.percep_loss = tf.reduce_mean(
            tf.abs(
                tf.reduce_mean(self.realA_percep, axis=3)
        ...
        for i in range(self.n_d):
            ...
            self.DB_fake_rec = self.discriminator(
                self.rec_realB,
                self.options,
                reuse=False,
                name=str(i) + "_reconstructionB",
            )
            self.DA_fake_rec = self.discriminator(
                self.rec_realA,
                self.options,
                reuse=False,
                name=str(i) + "_reconstructionA",
            )

            self.fake_B_rec = self.discriminator(
                self.rec_fakeB, self.options, reuse=False, name=str(i) + "_refineB"
            )
            self.fake_A_rec = self.discriminator(
                self.rec_fakeA, self.options, reuse=False, name=str(i) + "_refineA"
            )
            ...
            self.g_adv_total += (
                ...
                * 0.5
                + (
                    self.criterionGAN_list(
                        self.fake_B_rec, get_ones_like(self.fake_B_rec)
                    )
                    + self.criterionGAN_list(
                        self.fake_A_rec, get_ones_like(self.fake_A_rec)
                    )
                )
                * 0.5
                + (
                    self.criterionGAN_list(
                        self.DA_fake_rec, get_ones_like(self.DA_fake_rec)
                    )
                    + self.criterionGAN_list(
                        self.DB_fake_rec, get_ones_like(self.DB_fake_rec)
                    )
                )
                * 0.5
            )
            ...
            self.g_adv_rec += (
                self.criterionGAN_list(self.fake_B_rec, get_ones_like(self.fake_B_rec))
                + self.criterionGAN_list(
                    self.fake_A_rec, get_ones_like(self.fake_A_rec)
                )
            ) * 0.5
        ...
        self.g_loss = (
            ...
            + self.L1_lambda * abs_criterion(self.rec_realB, self.real_B)
            + self.percep_loss
            + self.g_cls_loss
        )
        ...
        for i in range(self.n_d):
            ...
            self.DB_real_rec = self.discriminator(
                self.real_B, self.options, reuse=True, name=str(i) + "_reconstructionB"
            )
            self.DA_real_rec = self.discriminator(
                self.real_A, self.options, reuse=True, name=str(i) + "_reconstructionA"
            )
            self.DB_fake_rec = self.discriminator(
                self.rec_B_sample,
                self.options,
            ...
            self.db_loss_real_rec = self.criterionGAN_list(
                self.DB_real_rec, get_ones_like(self.DB_real_rec)
            )
            self.db_loss_fake_rec = self.criterionGAN_list(
                self.DB_fake_rec, get_zeros_like(self.DB_fake_rec)
            )
            self.db_loss_rec = self.db_loss_real_rec * 0.5 + self.db_loss_fake_rec * 0.5
            self.da_loss_real_rec = self.criterionGAN_list(
                self.DA_real_rec, get_ones_like(self.DA_real_rec)
            )
            self.da_loss_fake_rec = self.criterionGAN_list(
                self.DA_fake_rec, get_zeros_like(self.DA_fake_rec)
            )
            ...
            self.real_B_adv = self.discriminator(
                self.real_B, self.options, reuse=True, name=str(i) + "_refineB"
            )
            self.real_A_adv = self.discriminator(
                self.real_A, self.options, reuse=True, name=str(i) + "_refineA"
            )
            self.DB_fake_recfake = self.discriminator(
                self.rec_fakeB_sample,
                self.options,
                reuse=True,
                name=str(i) + "_refineB",
            )
            self.DA_fake_recfake = self.discriminator(
                self.rec_fakeA_sample,
                self.options,
                reuse=True,
                name=str(i) + "_refineA",
            )
            self.db_loss_real_recfake = self.criterionGAN_list(
                self.real_B_adv, get_ones_like(self.real_B_adv)
            )
            self.db_loss_fake_recfake = self.criterionGAN_list(
                self.DB_fake_recfake, get_zeros_like(self.DB_fake_recfake)
            )
            self.da_loss_real_recfake = self.criterionGAN_list(
                self.real_A_adv, get_ones_like(self.real_A_adv)
            )
            self.da_loss_fake_recfake = self.criterionGAN_list(
                self.DA_fake_recfake, get_zeros_like(self.DA_fake_recfake)
            )
            self.db_loss_recfake = (
                self.db_loss_real_recfake * 0.5 + self.db_loss_fake_recfake * 0.5
            )
            self.da_loss_recfake = (
                self.da_loss_real_recfake * 0.5 + self.da_loss_fake_recfake * 0.5
            )
        ...
        self.testB, self.rec_testA, _ = self.generator(
            self.test_A, self.options, True, name="generatorA2B"
        )
        self.rec_cycle_A, self.refine_testB, _ = self.generator(
            self.testB, self.options, True, name="generatorB2A"
        )

        self.testA, self.rec_testB, _ = self.generator(
            self.test_B, self.options, True, name="generatorB2A"
        )
        self.rec_cycle_B, self.refine_testA, _ = self.generator(
        ...
        for i in range(self.n_d):
            self.R_vars = [
                var for var in t_vars if str(i) + "_reconstruction" in var.name
            ]
    ...
    def train(self, args):
        ...
        self.p_optim = tf.compat.v1.train.AdamOptimizer(
            self.lr, beta1=args.beta1
        ...
        self.rec_optim_item = []
        for i in range(self.n_d):
            self.rec_optim = tf.compat.v1.train.AdamOptimizer(
                self.lr, beta1=args.beta1
        ...
        for i in range(self.n_d):
            self.recfake_optim = tf.compat.v1.train.AdamOptimizer(
                self.lr, beta1=args.beta1
        ...
        for epoch in range(args.epoch):
            ...
            for idx in range(0, batch_idxs):
                ...
                for i in range(self.n_d):
                    ...
                    _, d_loss_recfake = self.sess.run(
                        ...
                        feed_dict={
                            ...
                            self.rec_fakeB_sample: rec_fake_B,
                            self.lr: lr,
                        },
                    )
                ...
                print(
                    (
                        "Epoch: [%2d] [%4d/%4d] time: %4.4f g_loss: %4.4f gan:%4.4f adv:%4.4f adv_rec:%4.4f adv_recfake:%4.4f pix_cycle:%4.4f pix_rec:%4.4f g_percep:%4.4f cls:%4.4f g_cls:%4.4f"
                        % (
                            epoch,
                            idx,
                            batch_idxs,
    ...
    def sample_model(self, sample_dir, epoch, idx):
        ...
        merge_A = np.concatenate(
            [real_B, fake_A, rec_B, rec_fakeA, rec_cycle_B], axis=2
        )
    ...
    def test(self, args):
        ...
        if self.load(args.checkpoint_dir):
            ...
            print(" [!] Load failed...")
        out_var, refine_var, in_var, rec_var, cycle_var = (
            (
                self.testB,
                self.refine_testB,
                self.test_A,
        ...
        for sample_file in sample_files:
            ...
            fake_img, refine_fake, rec_img, cycle_img = self.sess.run(
                [out_var, refine_var, rec_var, cycle_var],
                feed_dict={in_var: sample_image},
            )
            merge = np.concatenate(
                [sample_image, fake_img, refine_fake, rec_img, cycle_img], axis=2
            )
